...
|-------|---------|---------|
| `enabled` | *derived* | Read-only; `true` только когда включена crypto **и** задан wallet key (`merged.cryptoEnabled && Boolean(walletKey)`, `src/config.ts`). Не задавать в JSON. |
...
| `oracleFamilyUrl` | `https://oracles.modelmarket.dev/family` | Oracle-family endpoint перед LUMEN (`ARGUS_ORACLE_FAMILY_URL`). |
...
| `ARGUS_ORACLE_FAMILY_URL` | Override oracle-family / LUMEN endpoint. Заполняет и `warden.oracleFamilyUrl`, который не читает ни один шлюз WARDEN. |
