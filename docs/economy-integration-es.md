...
|-------|-------------|-------------|
| `enabled` | *derived* | Read-only; `true` solo cuando crypto está activo **y** hay wallet key (`merged.cryptoEnabled && Boolean(walletKey)`, `src/config.ts`). No establecer en JSON. |
...
| `ARGUS_ORACLE_FAMILY_URL` | Override del endpoint oracle-family / LUMEN. También rellena `warden.oracleFamilyUrl`, que ninguna puerta de WARDEN lee. |
