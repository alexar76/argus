...
| `allowUnknownServers` | `boolean` | `true`, or `false` under `AIFACTORY_PROD=1` | Permit connecting to servers the operator never declared — i.e. servers discovered from an `mcp.catalogs` entry, which carry `McpServerRef.catalog`. `false` is fail-closed: only servers listed under `mcp.servers` may connect. **Production:** with `AIFACTORY_PROD=1` this flips to `false` unless `argus.config.json` states it — a value you wrote is a decision and is never reversed underneath you, which is why `argus.config.example.json` deliberately omits the key rather than restating the default. `argus doctor` prints what production tightened. |
