...
> 🌐 Language: **English** · [Русский](./verifiable-conscience-ru.md) · [Español](./verifiable-conscience-es.md)
...
> **Honest scope.** The slash is **not** an open dispute endpoint, and it does not act on the
> number the disputer submits. `/self-bond/slash` is **operator-only** (`AIMARKET_ADMIN_TOKEN`)
> because it destroys another party's collateral, and the submitted `observed_spend_usd` is
> **capped at the spend the hub ledger actually debited** for the bonded wallet; if the hub
> holds no settlement record for that wallet it refuses (`422`) instead of slashing on an
> unverifiable claim. The hub still cannot see ARGUS's off-hub token spend — only what flowed
> through it — so it slashes on **hub-observable** values only, and the ceiling that decides a
> breach is set through a per-agent-credentialed endpoint, not a token every publisher holds.

### Hub endpoints

`Own credential` = the agent's own secret from `AIMARKET_PUBLISHER_TOKENS` (or the operator
token). Under the shared publish token these routes could not tell one caller from another,
so in production a hub with neither configured refuses them with `503` rather than accepting
it.

| Method | Path | Auth | Purpose |
|---|---|---|---|
| `POST` | `/ai-market/v2/supply/stake` | own credential / admin | credit collateral — in production every positive amount needs a `tx_hash` that is verified on-chain and single-use |
| `POST` | `/ai-market/v2/self-bond/register` | own credential / admin | stake-backed bond: agent_id, evm_address, ceiling_usd, bond_usd, commitment (it sets the ceiling a slash is measured against) |
| `POST` | `/ai-market/v2/self-bond/slash` | **admin only** | slash on breach: agent_id, observed_spend_usd (capped at hub-recorded settlement), evidence |
| `GET` | `/ai-market/v2/self-bond/{agent_id}` | public | read a bond's state |
