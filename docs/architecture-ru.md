...
| 4 | **MCP host + WARDEN** 🛡️ | Подключает MCP-серверы как инструменты, но только после прохождения цепочки ворот WARDEN. Статическое сканирование → threat feed → origin → pinning. | `@aimarket/warden/src/static-scan.ts`, `@aimarket/warden/src/threat-feed.ts`, `@aimarket/warden/src/origin.ts`, `@aimarket/warden/src/pinning.ts`, `@aimarket/warden/src/sandbox.ts` |
...
У слоя 4 вообще нет входов со стороны экономики. Его третьи ворота — ворота **origin**: они решают по локальному факту — объявлен сервер в `mcp.servers` или обнаружен по записи из `mcp.catalogs`, — поэтому файрволу не нужен оракул и он не открывает ни одного сокета во время проверки. Подробности в [security-warden.md](./security-warden-ru.md#шлюз-origin--и-шлюз-репутации-который-стоял-здесь-раньше).

---

## Место ARGUS в более широкой экосистеме

ARGUS — **узел на стороне спроса**. Factory 🏭 собирает агентов; **HESTIA** 🔥 — очаг, где процесс продавца слушает (не Hub); Hub 🛒 перечисляет анонсированные возможности; Oracles 🔮 (LUMEN и др.) оценивают доверие; Monitor 👽 наблюдает за сетью. ARGUS обнаруживает, оплачивает и вызывает эти возможности — и сам может зарегистрироваться как поставщик.

```mermaid
flowchart LR
  FACTORY["🏭 Factory<br/>builds agents"] -->|scaffold · deploy| HESTIA["🔥 HESTIA<br/>очаг · hosted-runtime"]
  HESTIA -->|явный announce| HUB["🛒 Hub / Marketplace<br/>AI Market Protocol v2"]
  ...
  ARGUS -->|"бесплатные чтения · платные вызовы оракулов"| ORACLES
```
