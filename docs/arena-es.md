...
| **Rango de reputación subido** (percentil LUMEN sube un tier) | **100** | delta `lumen.reputation@v1` | ⏳ pending — ver la precondición LUMEN abajo |
...
> conectarse.
>
> El rango de reputación LUMEN es el caso más lejano: `scoreEntity` se llama hoy sin
> aristas de confianza y ningún despliegue publica un grafo de confianza contra el que
> puntuar, así que **no es obtenible hoy** — ni su XP ni el badge *Trusted*.
...
| **Trusted** | 🔮 | Alcanzar el **top 50%** de reputación LUMEN | `lumen.reputation@v1` | ⏳ pending — necesita un grafo de confianza que ningún despliegue publica |
...
| `lumenRank` | string (e.g. `Top 18%`) | percentil `lumen.reputation@v1` — en blanco hoy; ver la precondición LUMEN en §2 |
...
La línea `🔮 LUMEN` de arriba es una maqueta de una tarjeta poblada. En una tarjeta real
va en blanco: una puntuación LUMEN necesita un grafo de confianza, y nada lo suministra
(ver §2).
...
- **Las stats se calculan localmente.** XP, levels, streaks y badges se derivan
  ...
  firmados** más el **LUMEN score** cuando exista un grafo de confianza — hoy no
  existe ninguno. No hay telemetry callback.
- **Difícil de falsificar.** Los episodes los escribe el bounded agent loop como
  ...
  LUMEN sería una lectura **verificable** del oracle (`graph_commitment`), pero hoy va
  en blanco: puntuar requiere un grafo de confianza que ningún despliegue publica. Para
...
| reputation rank | **LUMEN** `lumen.reputation@v1` (`src/economy/lumen.ts`) | ⏳ pending — LUMEN solo puntúa cuando quien llama suministra aristas de confianza, y ningún despliegue publica un grafo |
...
- ⏳ **Pending native economy integration:** XP de earnings/sales y los badges
  **Rainmaker / Whale**; XP de lotería y **First Blood / Lucky**; XP ACEX;
  XP de rango LUMEN y **Trusted** + el campo `lumenRank` de la tarjeta (bloqueados
  además por la falta de un grafo de confianza publicado);
