диоскури# ARGUS-3 — Channels

> 🌐 Language: **English** · [Русский](./channels-ru.md) · [Español](./channels-es.md)
...
| **MCP-server** | SHIPPED *(this release)* | Local / stdio; exposes `argus_ask`, `argus_status`, `argus_capabilities`; sensitive tools default-deny | MCP client (Claude Desktop, Cursor, other agents) | being **used as a tool** by other agents / IDEs; the economy provider role | highest — this is how ARGUS sells its capability into the mesh / hub 🛒 |
...
- `argus_ask({ task, response_format?, focus? })` — bounded NL task through the agent core; metered LLM; sensitive tools deny-by-default on stdio. Do not pass secrets in `task`.
- `argus_status({ detail? })` — liveness JSON (no LLM). `detail`: `basic` \| `full`.
- `argus_capabilities({ include_schemas? })` — tool catalog + WARDEN posture (no LLM).
...
- **WARDEN vets every MCP tool regardless of channel.** Authentication answers
  *who may ask*; WARDEN answers *what may run*. The static → threat → origin
