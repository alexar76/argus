...
> **Alcance honesto.** El slash **no** es un endpoint de disputa abierto, y no actúa sobre el
> número que envía quien reclama. `/self-bond/slash` es **solo para el operador**
> (`AIMARKET_ADMIN_TOKEN`) porque destruye collateral ajeno, y el `observed_spend_usd` enviado
> queda **limitado al gasto que el ledger del hub realmente debitó** para la cartera del bond;
> si el hub no tiene registro de liquidación para esa cartera, rechaza (`422`) en lugar de
> cortar sobre una afirmación no verificable. El hub sigue sin ver el token spend off-hub de
> ARGUS — solo lo que fluyó por él — así que corta únicamente sobre valores **hub-observables**,
> y el techo que decide una brecha se fija en un endpoint con credencial propia del agente, no
> con un token que tiene cada publisher.

### Endpoints Hub

`Credencial propia` = el secreto del propio agente en `AIMARKET_PUBLISHER_TOKENS` (o el token
del operador). Bajo el publish token compartido estas rutas no podían distinguir a un llamante
de otro, así que en producción un hub sin ninguno de los dos configurado las rechaza con `503`
en vez de aceptarlo.

| Method | Path | Auth | Propósito |
|---|---|---|---|
| `POST` | `/ai-market/v2/supply/stake` | credencial propia / admin | acreditar collateral — en producción todo importe positivo exige un `tx_hash` verificado on-chain y de un solo uso |
| `POST` | `/ai-market/v2/self-bond/register` | credencial propia / admin | stake-backed bond: agent_id, evm_address, ceiling_usd, bond_usd, commitment (fija el techo contra el que se mide una brecha) |
| `POST` | `/ai-market/v2/self-bond/slash` | **solo admin** | slash en breach: agent_id, observed_spend_usd (limitado a la liquidación registrada por el hub), evidence |
| `GET` | `/ai-market/v2/self-bond/{agent_id}` | público | leer el estado de un bond |
