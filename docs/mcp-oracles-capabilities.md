...
| **Lumen** | Reputation / trust (PageRank / EigenTrust) | `lumen.reputation@v1` — needs a caller-supplied trust graph; no ARGUS code path builds one today |
...
|---------|---------|---------|
| `warden.oracleFamilyUrl` | `https://oracles.modelmarket.dev/family` | Oracle family fronting LUMEN — read by the runtime's `LumenOracle`, not by any WARDEN gate |
