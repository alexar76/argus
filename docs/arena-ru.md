...
лотерее, сделки ACEX, репутацию LUMEN (условие — см. §2), блокировки
MCP-серверов WARDEN — в
...
| **Ранг репутации вырос** (LUMEN percentile на tier выше) | **100** | delta `lumen.reputation@v1` | ⏳ pending — см. условие LUMEN ниже |
...
>
> **Условие LUMEN.** Строка с рангом репутации ждёт не только «проводки»:
> `LumenOracle.scoreEntity` доходит до оракула только если вызывающая сторона
> передаст рёбра доверия, а ни один путь в коде ARGUS такой граф не строит. Пока
> какая-нибудь инсталляция не опубликует рёбра доверия, LUMEN-score получить
> нельзя вообще — поэтому ранг, badge **Trusted** и поле `lumenRank` на карточке
> остаются пустыми.
...
| **Trusted** | 🔮 | Попадание в **top 50%** LUMEN reputation | `lumen.reputation@v1` | ⏳ pending — сегодня получить нельзя (см. §2) |
...
| `lumenRank` | string (e.g. `Top 18%`) | percentile `lumen.reputation@v1` — сегодня пусто; см. условие LUMEN в §2 |
...
Строка `🔮 LUMEN` выше — макет заполненной карточки. На реальной карточке она
пуста: LUMEN-score требует графа доверия, а его никто не поставляет (см. §2).
...
- **Статистика вычисляется локально.** XP, levels, streaks и badges выводятся из
  **собственного memory store** агента (episodes) плюс **подписанные economy
  receipts**. Нет telemetry callback.
- **Сложно подделать.** Episodes пишет bounded agent loop как побочный эффект
  ...
  escrow/Mesh, а не self-reported числа. Чтобы раздуть «earned», нужно подделать
  подпись escrow — т.е. нельзя. LUMEN rank, если его когда-нибудь станет
  возможно получить, был бы **верифицируемым** oracle reading
  (`graph_commitment`), а не self-reported числом.
...
| reputation rank | **LUMEN** `lumen.reputation@v1` (`src/economy/lumen.ts`; WARDEN его не использует) | ⏳ pending — нужен граф доверия, которого не публикует ни одна инсталляция |
...
- ⏳ **Pending native economy integration:** earnings/sales XP и badges
  **Rainmaker / Whale**; lottery XP и **First Blood / Lucky**; ACEX XP;
  LUMEN-rank XP и **Trusted** + поле `lumenRank` на карточке (требуют графа
  доверия, которого не публикует ни одна инсталляция); PNG rasterising;
