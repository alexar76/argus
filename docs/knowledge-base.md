...
> 🌐 Language: **English** · [Русский](./knowledge-base-ru.md) · [Español](./knowledge-base-es.md)
...
> **knowledge-base** · [architecture](./architecture.md) · [security-warden](./security-warden.md) · [economy-integration](./economy-integration.md) · [mcp-oracles-capabilities](./mcp-oracles-capabilities.md) · [token-economy](./token-economy.md) · [autonomy](./autonomy.md) · [channels](./channels.md) · [arena](./arena.md)
...
3. **Check the server's origin** — did the operator declare it under `mcp.servers`,
   or was it discovered from a remote `mcp.catalogs` entry? Under
   `allowUnknownServers: false` the catalog-discovered kind is blocked; a declared
   server always passes. A local fact, so no oracle and no network are involved.
...
flowchart LR
  ...
  subgraph ARGUS["ARGUS (single standalone agent)"]
    ...
    subgraph WARDEN["🛡️ WARDEN — firewall module inside ARGUS"]
      direction LR
      G1["1 · static-scan"] --> G2["2 · threat feed"] --> G3["3 · origin"] --> G4["4 · def-pinning"] --> G5["5 · sensitive → owner approval"]
    end
...
it has no authority over ARGUS. It consults nothing outside the machine either: every
gate decides from the advertised tool definitions, your config, and the local pin
store.
...
```mermaid
flowchart LR
  FACTORY["🏭 Factory<br/>builds & ships products"] -->|scaffold · deploy| HESTIA["🔥 HESTIA<br/>hearth · hosted runtime"]
  HESTIA -->|explicit announce| HUB["🛒 AIMarket Hub<br/>+ Protocol v2 (broker)"]
  ORACLES["🔮 Oracles (17)<br/>Platon · Chronos · Lattice · Murmuration · Lumen · Colony · Turing<br/>Percola · Fermat · Ablation · Landauer · Sortes · Gauss · Aestus · Betti · Kantor · Fourier"] --> HUB
...
- 🔥 **HESTIA** — the hearth: isolated hosted runtime where a seller process
  actually listens. Not the Hub catalogue, not Factory, not a job board. Signed
  deploy; empty roster ≠ empty market; announce is explicit.
- 🛒 **AIMarket Hub + Protocol v2** — the marketplace/broker. Capabilities are
  ...
  itself as a seller.
- 🔮 **Oracles (17)** — verifiable math services ARGUS can call and pay for:
  ...
  - **LUMEN** — reputation/trust via PageRank/EigenTrust (`lumen.reputation@v1`).
    Scoring needs a caller-supplied trust graph; no ARGUS code path builds one
    today, so `argus passport` prints `unattested` and WARDEN does not use it.
  - **Chronos** — verifiable delay function (VDF) (`chronos.eval@v1`, `chronos.verify@v1`).
  ...
  - **Colony** — combinatorial optimization (`colony.optimize@v1`).
  - **Turing** — blue-noise structured sampling (`turing.bluenoise@v1`).
  - **Percola, Fermat, Ablation, Landauer** — resilience, routing, cascade-risk, thermodynamic audit.
  - **Sortes** — true ECVRF (RFC 9381) ungrindable verifiable randomness; one valid output per (key, input), verifiable offline from an 80-byte proof (`sortes.draw@v1`, `sortes.verify@v1`).
  - **Gauss** — Gaussian-process regression: calibrated posterior mean + honest uncertainty everywhere, plus the best next point to sample (`gauss.field@v1`, `gauss.suggest@v1`, `gauss.verify@v1`).
  - **Aestus** — RSW time-lock puzzles: seal data nobody can open before ~T sequential squarings elapse, then anyone can — no trapdoor holder (`aestus.seal@v1`, `aestus.open@v1`, `aestus.verify@v1`).
  - **Betti** — persistent homology (Vietoris-Rips): Betti numbers across scale plus a bottleneck-distance drift alarm — the shape of a point cloud (`betti.homology@v1`, `betti.distance@v1`).
  - **Kantor** — exact optimal transport (Wasserstein) via min-cost flow, returning the plan plus Kantorovich dual potentials as an O(m·n)-verifiable certificate (`kantor.transport@v1`, `kantor.verify@v1`).
  - **Fourier** — graph-spectral analysis: Laplacian spectrum, algebraic connectivity (Fiedler value/vector), spectral cut & conductance, embedding (`fourier.spectrum@v1`, `fourier.verify@v1`).
  - Full capability list: **[mcp-oracles-capabilities.md](./mcp-oracles-capabilities.md)**.
...
|---|---|---|
| **Call oracles** | `oracle_call` / `argus oracle <verb>` — all **17** AICOM oracles (Platon, Chronos, Lattice, Murmuration, Lumen, Colony, Turing, Percola, Fermat, Ablation, Landauer, Sortes, Gauss, Aestus, Betti, Kantor, Fourier). Free off-chain reads. | **No** |
...
| **Register + sell capabilities in the Mesh** | `argus economy register` + `argus serve` / `argus mcp` — list on Hub, earn USDC per invoke. | **Yes** |
| **Defend against malicious MCP servers** | The WARDEN firewall vets every third-party MCP server (static scan → threat feed → origin → pinning → sensitive-tool approval). | **No** — WARDEN works wallet-free |
