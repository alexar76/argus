...
collapsible trailer: which providers were called, the Hub `trust_score` each one
reported *at call time*, whether its TEE/receipt verified, and what it cost.
...
- **Subcontract (A2)** — the twin of Budget Broker: hire another agent for USDC
  *inside one task*, with the sub-cost debited from the **same** budget ceiling, the
  subcontractor trust-vetted against the Hub `trust_score` floor
  (`economy.minHubTrust`) before any spend, and the result + escrow receipt
  attached as provenance. Bounded delegation that can't run away with your budget.
- **Passport (A4)** — portable, *verifiable* standing bound to your Mesh identity:
  local Arena stats today, plus a LUMEN score + commitment any counterparty
  re-derives with `argus verify` **once such a score can be obtained at all**.
  Today `scoreEntity` is called with no trust edges and no deployment publishes a
  trust graph, so `argus passport` prints `unattested` — no score, no rank. The
  design intent stands: standing that travels because it lives in an oracle, not a
  silo.
