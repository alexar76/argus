...
2. **Проверяет ленту угроз** — встроенный deny-list известных вредоносных
   ...
   pull-only).
3. **Проверяет происхождение сервера** (шлюз `origin`) — объявил его оператор в
   `mcp.servers` или он обнаружен в удалённом каталоге из `mcp.catalogs`. Только
   второй вид считается «неизвестным»: при `allowUnknownServers: false` он
   блокируется с кодом `SERVER_UNDECLARED`, а объявленный сервер допускается
   всегда. Это локальный факт — сеть и оракул не нужны.
...
направляет *агентов*, и у него нет власти над ARGUS. Он вообще не обращается ни к
какому оракулу доверия: каждый его шлюз решает по локальным данным — определения
инструментов, лента угроз на диске, происхождение объявления сервера и
закреплённый снимок.
...
```mermaid
flowchart LR
  FACTORY["🏭 Factory<br/>builds & ships products"] -->|scaffold · deploy| HESTIA["🔥 HESTIA<br/>очаг · hosted-runtime"]
  HESTIA -->|явный announce| HUB["🛒 AIMarket Hub<br/>+ Protocol v2 (broker)"]
...
- 🔥 **HESTIA** — очаг: изолированный hosted-runtime, где процесс продавца
  слушает. Не каталог Hub, не Factory, не доска работ. Подписанный деплой;
  пустой roster ≠ пустой рынок; announce явный.
...
- 🔮 **Оракулы (17)** — верифицируемые математические сервисы, которые ARGUS может вызывать и оплачивать:
  - **Platon** — случайность/VRF, маяк случайности, commit-reveal и заземлённый LLM-«ask».
  - **Lumen** — репутация/доверие через PageRank/EigenTrust (`lumen.reputation@v1`); в ARGUS его читает только команда `argus passport`, и она получает score лишь при наличии графа доверия. WARDEN его не использует.
  - **Chronos** — verifiable delay function (VDF) (`chronos.eval@v1`, `chronos.verify@v1`).
  - **Lattice** — квазислучайные последовательности с низким расхождением (`lattice.sequence@v1`).
  - **Murmuration** — робастный консенсус (`murmuration.aggregate@v1`).
  - **Colony** — комбинаторная оптимизация (`colony.optimize@v1`).
  - **Turing** — blue-noise сэмплирование (`turing.bluenoise@v1`).
  - **Percola, Fermat, Ablation, Landauer** — устойчивость сети, маршрутизация, каскадный риск, термодинамический аудит.
  - **Sortes** — ECVRF (RFC 9381) верифицируемая случайность (`sortes.draw@v1`, `sortes.verify@v1`).
  - **Gauss** — гауссовские процессы: posterior + uncertainty + next point (`gauss.field@v1`, `gauss.suggest@v1`).
  - **Aestus** — RSW time-lock головоломки (`aestus.seal@v1`, `aestus.open@v1`).
  - **Betti** — персистентная гомология (`betti.homology@v1`, `betti.distance@v1`).
  - **Kantor** — оптимальный транспорт / Wasserstein (`kantor.transport@v1`, `kantor.verify@v1`).
  - **Fourier** — спектральный анализ графа (`fourier.spectrum@v1`, `fourier.verify@v1`).
  - Полный список: **[mcp-oracles-capabilities.md](./mcp-oracles-capabilities.md)**.
...
| **Защита от вредоносных MCP-серверов** | Файрвол WARDEN проверяет каждый сторонний MCP-сервер (статическое сканирование → лента угроз → `origin` → закрепление → одобрение чувствительных инструментов). | **Нет** — WARDEN работает без кошелька |
