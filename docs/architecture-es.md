...
| 4 | **MCP host + WARDEN** 🛡️ | Conecta servidores MCP como herramientas, pero solo después de que la cadena de puertas WARDEN las autorice. Escaneo estático → threat feed → origin → pinning. | `@aimarket/warden/src/static-scan.ts`, `@aimarket/warden/src/threat-feed.ts`, `@aimarket/warden/src/origin.ts`, `@aimarket/warden/src/pinning.ts`, `@aimarket/warden/src/sandbox.ts` |
...
flowchart TB
  subgraph OFFLINE["Works offline — no AICOM, no wallet"]
    direction TB
    L4["🛡️ Layer 4 · MCP host + WARDEN<br/>static-scan · threat-feed · origin · pinning"]
...
La capa 4 no tiene ninguna entrada del lado de la economía. Su tercera puerta es la puerta **origin**, que decide a partir de un hecho local — si un servidor fue declarado bajo `mcp.servers` o descubierto desde una entrada de `mcp.catalogs` — así que el firewall no necesita oráculo y no abre ningún socket mientras hace el vetting. Detalles en [security-warden.md](./security-warden.md#the-origin-gate-and-the-reputation-gate-that-used-to-be-here).

---

## Dónde se sitúa ARGUS en el ecosistema más amplio

ARGUS es el **nodo del lado de la demanda**. Factory 🏭 arma agentes; **HESTIA** 🔥 es el hogar donde el proceso del vendedor escucha (no el Hub); Hub 🛒 lista las capacidades anunciadas; Oracles 🔮 (LUMEN, etc.) puntúan la confianza; Monitor 👽 observa la red. ARGUS descubre, paga e invoca esas capacidades — y puede registrarse como proveedor.

```mermaid
flowchart LR
  FACTORY["🏭 Factory<br/>builds agents"] -->|scaffold · deploy| HESTIA["🔥 HESTIA<br/>hogar · runtime alojado"]
  HESTIA -->|anuncio explícito| HUB["🛒 Hub / Marketplace<br/>AI Market Protocol v2"]
  ...
  ARGUS -->|"oracle_call · paid reads"| ORACLES
```
...
src/
  ...
  warden/               🛡️ MCP firewall (see security-warden.md)
    ...
    origin.ts           OriginGate — declared vs catalog-discovered
  ...
  economy/              🛒 opt-in (see economy-integration.md)
    ...
    lumen.ts            LumenOracle — TrustOracle; scores only when the caller
                        supplies trust edges (no caller does today)
  tools/
