...
Voer de one-liner installer uit. Controleert Node.js 20+, installeert `@aimarket/argus` globaal, maakt `~/.argus/agent` aan en start `argus setup`. Wizard: crypto-wallet (optioneel, standaard UIT), omgevingsmodus, LLM-provider, optionele Telegram, HTTP-token.
