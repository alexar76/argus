...
Einzeiler-Installer ausführen. Prüft Node.js 20+, installiert `@alexar76/argus3` global, erstellt `~/.argus/agent` und startet `argus setup`. Assistent: Krypto-Wallet (optional, standardmäßig AUS), Umgebungsmodus, LLM-Anbieter, optionales Telegram, optionaler HTTP-Token.
