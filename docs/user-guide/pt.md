...
Execute o instalador de uma linha. Verifica Node.js 20+, instala `@alexar76/argus3` globalmente, cria `~/.argus/agent` e inicia `argus setup`. O assistente cobre: carteira cripto (opcional, OFF por padrão), modo de ambiente, provedor LLM, Telegram opcional, token HTTP opcional.
