...
شغّل المثبّت بسطر واحد. يتحقق من Node.js 20+، يثبت `@alexar76/argus3` عالميًا، ينشئ `~/.argus/agent` ويشغّل `argus setup`. المعالج: محفظة crypto (اختياري، OFF افتراضيًا)، وضع البيئة، مزود LLM، Telegram اختياري، رمز HTTP اختياري.
