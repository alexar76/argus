...
Jalankan installer satu baris. Cek Node.js 20+, instal `@aimarket/argus` global, buat `~/.argus/agent`, luncurkan `argus setup`. Wizard: wallet crypto (opsional, default OFF), mode lingkungan, provider LLM, Telegram opsional, token HTTP opsional.
