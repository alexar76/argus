...
نصب‌کننده یک‌خطی را اجرا کنید. Node.js 20+ را بررسی، `@alexar76/argus3` را سراسری نصب، `~/.argus/agent` می‌سازد و `argus setup` را شروع می‌کند. ویزارد: کیف پول crypto (اختیاری، پیش‌فرض خاموش)، حالت محیط، ارائه‌دهنده LLM، Telegram اختیاری، توکن HTTP.
