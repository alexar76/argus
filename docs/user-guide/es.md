...
WARDEN decide con hechos locales: static-scan de las tool-defs → threat feed → origin →
def-pinning. La puerta `origin` distingue dos tipos de servidor: los que declaras bajo
`mcp.servers` y los descubiertos desde `mcp.catalogs`. Con `allowUnknownServers: false`
(fail-closed) un servidor descubierto en un catálogo se bloquea con `SERVER_UNDECLARED`,
mientras que los declarados siempre pasan. Ninguna puerta lee `oracleFamilyUrl` — es el
endpoint del `LumenOracle` que usa `argus passport`.
