...
Ejecuta el instalador de una línea. Comprueba Node.js 20+, instala `@aimarket/argus` globalmente, crea `~/.argus/agent` y lanza `argus setup`. El asistente cubre: wallet cripto (opcional, OFF por defecto), modo de entorno, proveedor LLM, Telegram opcional, token HTTP opcional.
