...
WARDEN vets every MCP server locally — static scan → threat feed → origin →
pinning. Nothing is asked of the network. `allowUnknownServers: false` restricts
connections to the servers you listed under `mcp.servers`: anything discovered
from a remote `mcp.catalogs` entry is blocked with `SERVER_UNDECLARED`, while your
own declared servers still connect, so fail-closed does not lock you out.

No WARDEN gate reads `oracleFamilyUrl`. The key still exists because the runtime
builds a `LumenOracle` from it for the `argus passport` command; the firewall
decides from local facts only.
