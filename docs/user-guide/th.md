...
รันตัวติดตั้งบรรทัดเดียว ตรวจ Node.js 20+ ติดตั้ง `@alexar76/argus3` แบบ global สร้าง `~/.argus/agent` เริ่ม `argus setup` ตัวช่วย: crypto wallet (ไม่บังคับ ปิดเป็นค่าเริ่มต้น) โหมดสภาพแวดล้อม LLM provider Telegram token HTTP
