...
Esegui l'installer monoriga. Verifica Node.js 20+, installa `@alexar76/argus3` globalmente, crea `~/.argus/agent` e avvia `argus setup`. Wizard: wallet crypto (opzionale, OFF di default), modalità ambiente, provider LLM, Telegram opzionale, token HTTP opzionale.
