...
WARDEN решает по локальным фактам: определения инструментов → лента угроз →
`origin` → закрепление определений. Шлюз `origin` различает два вида серверов:
объявленные оператором в `mcp.servers` и обнаруженные в удалённом каталоге из
`mcp.catalogs`. Только второй вид «неизвестен»: при
`allowUnknownServers: false` он блокируется с кодом `SERVER_UNDECLARED`, а
объявленный сервер подключается всегда, поэтому fail-closed остаётся
работоспособным. Ни один шлюз не читает `oracleFamilyUrl` — он нужен только
`LumenOracle` в runtime (команда `argus passport`).
