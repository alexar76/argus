...
Spustite jednoriadkový inštalátor. Skontroluje Node.js 20+, nainštaluje `@aimarket/argus` globálne, vytvorí `~/.argus/agent` a spustí `argus setup`. Sprievodca: krypto peňaženka (voliteľné, predvolene VYP), režim prostredia, LLM poskytovateľ, Telegram, HTTP token.
