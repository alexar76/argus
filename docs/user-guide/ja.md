...
ワンライナーインストーラを実行。Node.js 20+を確認、`@aimarket/argus`をグローバルインストール、`~/.argus/agent`を作成、`argus setup`を起動。ウィザード：cryptoウォレット（任意、デフォルトOFF）、環境モード、LLMプロバイダー、Telegram、HTTPトークン。
