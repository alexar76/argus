...
> 🌐 Language: **English** · [Русский](./arena-ru.md) · [Español](./arena-es.md)
...
trades, LUMEN reputation (see the precondition in §2), MCP servers blocked — into
mechanics proven to engage a young, international audience: Duolingo-style
streaks 🔥, Spotify-Wrapped-style shareable cards, and gaming-style rank cards.
...
| **Reputation rank gained** (LUMEN percentile up a tier) | **100** | `lumen.reputation@v1` delta | ⏳ pending — see the LUMEN precondition below |
...
>
> **LUMEN precondition.** The reputation-rank row is pending on more than
> plumbing: `LumenOracle.scoreEntity` only reaches the oracle when a caller
> supplies trust edges, and no ARGUS code path builds that graph. Until some
> deployment publishes trust edges, a LUMEN score cannot be earned at all — so
> the rank, the **Trusted** badge and the card's `lumenRank` field stay empty.
...
| **Trusted** | 🔮 | Reach the **top 50%** LUMEN reputation | `lumen.reputation@v1` | ⏳ pending — not obtainable today (see §2) |
...
| `lumenRank` | string (e.g. `Top 18%`) | `lumen.reputation@v1` percentile — blank today; see the LUMEN precondition in §2 |
...
The `🔮 LUMEN` line above is a mock-up of a populated card. On a real card it is
blank: a LUMEN score needs a trust graph, and nothing supplies one (see §2).
...
- **Stats are computed locally.** XP, levels, streaks, and badges are derived
  ...
  receipts**. There is no telemetry callback.
- **Hard to fake.** Episodes are written by the bounded agent loop as a
  ...
  the AIMarket escrow/Mesh, not self-reported numbers. To inflate "earned" you'd
  have to forge an escrow signature — i.e. you can't. A LUMEN rank, if it ever
  becomes obtainable, would be a **verifiable** oracle reading
  (`graph_commitment`) rather than a self-reported number.
...
| WARDEN blocks | WARDEN gate reports (`@aimarket/warden/src/`) | ✅ today |
...
| reputation rank | **LUMEN** `lumen.reputation@v1` (`src/economy/lumen.ts`) | ⏳ not obtainable — `scoreEntity` is called with no trust edges, and no deployment publishes a graph to score |
...
- ⏳ **Pending native economy integration:** earnings/sales XP and the
  **Rainmaker / Whale** badges; lottery XP and **First Blood / Lucky**; ACEX XP;
  ...
  none is on the critical path. The LUMEN items wait on something outside ARGUS:
  a published trust graph to be scored against (§2).
