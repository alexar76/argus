...
| 4 | **MCP host + WARDEN** 🛡️ | Bridges MCP servers as tools, but only after the WARDEN gate chain clears them. Static scan → threat feed → origin → pinning. | `@aimarket/warden/src/static-scan.ts`, `@aimarket/warden/src/threat-feed.ts`, `@aimarket/warden/src/origin.ts`, `@aimarket/warden/src/pinning.ts`, `@aimarket/warden/src/sandbox.ts` |
...
No gate in Layer 4 takes an economy-side input. The third slot is the **origin**
gate, which reads whether a server was declared under `mcp.servers` or discovered
from an `mcp.catalogs` entry — a local fact, no oracle and no network. (It replaced
a reputation gate that asked LUMEN for a score without ever supplying trust edges,
so no request was ever made.) Details in
...
ARGUS is the **demand-side node**. The Factory 🏭 scaffolds agents; **HESTIA** 🔥
is the hearth where a seller process listens (not the Hub); the Hub 🛒
lists announced capabilities; the Oracles 🔮 (LUMEN et al.) score trust; the
...
```mermaid
flowchart LR
  FACTORY["🏭 Factory<br/>builds agents"] -->|scaffold · deploy| HESTIA["🔥 HESTIA<br/>hearth · hosted runtime"]
  HESTIA -->|explicit announce| HUB["🛒 Hub / Marketplace<br/>AI Market Protocol v2"]
  ...
  ARGUS -->|"free reads · paid oracle calls"| ORACLES
```
