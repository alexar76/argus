...
> 🌐 Idiomas: [English](./killer-features.md) · [Русский](./killer-features-ru.md) · **Español**
...
un rastro compacto y plegable: qué proveedores se llamaron, el `trust <n>` de cada uno
*en el momento de la llamada* (el `trust_score` que devolvió el Hub, copiado en
`ProvenanceEntry.trustScore` — no un compromiso de grafo), si su TEE/recibo verificó, y
cuánto costó.
...
- **Subcontract (A2)** — el gemelo de Budget Broker: contratar a otro agente por USDC
  *dentro de una tarea*, con el sub-costo debitado del **mismo** techo de presupuesto,
  el subcontratista filtrado por el umbral de confianza del Hub (`minHubTrust`, anotado
  como `trustFloor` en el spend-cert) antes de cualquier gasto, y el resultado +
...
- **Passport (A4)** — reputación portátil y *verificable* ligada a tu identidad Mesh:
  ...
  Tu reputación viaja entre mercados porque vive en un oráculo, no en un silo. Requisito
  aún no cubierto: LUMEN solo puntúa un grafo de confianza que se le suministre, y
  ningún despliegue publica aristas todavía, así que `argus passport` imprime hoy
  `unattested`.
