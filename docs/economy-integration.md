...
> 🌐 Language: **English** · [Русский](./economy-integration-ru.md) · [Español](./economy-integration-es.md)
...
>
> **Use case — third-party operator:** [run your own ARGUS on AICOM](./use-case-external-operator.md) · [RU](./use-case-external-operator-ru.md) · [ES](./use-case-external-operator-es.md)
...
|-------|---------|---------|
| `enabled` | *derived* | Read-only; `true` only when crypto is on **and** a wallet key is set (`merged.cryptoEnabled && Boolean(walletKey)`, `src/config.ts`). Do not set in JSON. |
...
- **Spend tools** require their own prerequisites: `acex_trade` needs
  `economy.acexEnabled` + a wallet; `lottery_enter` (unpaid work seat, default
  door) needs a wallet plus an agent credential from its home Hub;
  `lottery_buy` (paid tickets, off on LIVE) needs a wallet; all three
  are WARDEN-sensitive (per-call approval). `hub_invoke` (real paid USDC settlement)
...
[../../docs/private-evm-deployment.md](https://github.com/alexar76/aicom/blob/main/docs/private-evm-deployment.md).
...
| `ARGUS_LOTTERY_HUB_URL` | Home Hub that proves ecosystem participation; defaults to `ARGUS_HUB_URL`. |
| `ARGUS_LOTTERY_AGENT_ID` / `ARGUS_LOTTERY_AGENT_TOKEN` | Agent identity and its home-Hub credential. The token is sent only to that Hub, never to the lottery relayer. |
| `ARGUS_MESH_URL` | Override the Service Mesh endpoint. |
| `ARGUS_ORACLE_FAMILY_URL` | Override the oracle-family / LUMEN endpoint (sets both `economy.oracleFamilyUrl` and `warden.oracleFamilyUrl`; no WARDEN gate reads it). |
