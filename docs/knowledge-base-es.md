...
3. **Comprobar la procedencia del servidor** — declarado por el operador bajo
   `mcp.servers`, o descubierto desde un catálogo remoto bajo `mcp.catalogs`. Con
   `allowUnknownServers: false` un servidor descubierto en un catálogo se bloquea
   (`SERVER_UNDECLARED`) y los declarados siempre pasan. Es un hecho local: sin red y
   sin oráculo.
...
flowchart LR
  ...
  subgraph ARGUS["ARGUS (agente único e independiente)"]
    ...
    subgraph WARDEN["🛡️ WARDEN — módulo firewall dentro de ARGUS"]
      direction LR
      G1["1 · escaneo estático"] --> G2["2 · feed de amenazas"] --> G3["3 · origin"] --> G4["4 · fijado de def"] --> G5["5 · sensible → aprobación del dueño"]
    end
...
*agentes*, y no tiene autoridad sobre ARGUS. No consulta ningún oráculo: cada puerta
decide a partir de evidencia que ARGUS ya tiene localmente — las tool-defs anunciadas,
el threat feed en disco, la procedencia de la declaración del servidor y el snapshot
fijado.
...
```mermaid
flowchart LR
  FACTORY["🏭 Factory<br/>construye y entrega productos"] -->|scaffold · deploy| HESTIA["🔥 HESTIA<br/>hogar · runtime alojado"]
  HESTIA -->|anuncio explícito| HUB["🛒 AIMarket Hub<br/>+ Protocol v2 (bróker)"]
...
- 🔥 **HESTIA** — el hogar: runtime aislado donde el proceso del vendedor
  escucha. No es el catálogo del Hub, ni Factory, ni un tablón. Despliegue
  firmado; roster vacío ≠ mercado vacío; el anuncio es explícito.
...
- 🔮 **Oráculos (17)** — servicios matemáticos verificables que ARGUS puede llamar y pagar:
  ...
  - **Lumen** — reputación/confianza vía PageRank/EigenTrust (`lumen.reputation@v1`); puntúa el grafo de confianza que le pasa quien llama. WARDEN no lo consulta.
...
| **Defender contra servidores MCP maliciosos** | El firewall WARDEN examina cada servidor MCP de terceros (escaneo estático → feed de amenazas → origin → fijado → aprobación de herramientas sensibles). | **No** — WARDEN funciona sin wallet |
