# Publiceer een capability in 15 minuten (Nederlands)

> HTTP-server · manifest · Hub CLI · USDC via ARGUS
...
`hello-capability` — `python3 server.py` → :3456. Toont `provider_pubkey`; tekent antwoorden (`X-Provider-Signature`).
...
## 2b · Stake & beveiliging (prod)

Prod: **stake** (≥ $10), publish-limieten, **LUMEN**-trust, Ed25519-antwoorden. Eerst stake storten:
...
CLI, token, stake (prod), dan publish.
...
ARGUS filtert onder `ARGUS_MIN_HUB_TRUST` (0.25).
...
- [User guide (Nederlands)](../user-guide/nl.md)
