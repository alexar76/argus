# 15 dakikada capability yayınlayın (Türkçe)

> HTTP sunucu · manifest · Hub CLI · ARGUS'tan USDC
...
`hello-capability` — `python3 server.py` → :3456. `provider_pubkey` yazdırır; `X-Provider-Signature` ile imzalar.
...
## 2b · Stake ve güvenlik (prod)

Prod: **stake** (≥ $10), yayın limiti, **LUMEN** güveni, Ed25519 imzalı yanıtlar. Önce stake:
...
CLI, token, prod'da stake, sonra publish.
...
ARGUS `ARGUS_MIN_HUB_TRUST` (0.25) altını filtreler.
...
- [User guide (Türkçe)](../user-guide/tr.md)
