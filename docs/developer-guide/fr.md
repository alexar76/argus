# Publiez une capability en 15 minutes (Français)

> serveur HTTP · manifeste · CLI Hub · USDC via ARGUS
...
`hello-capability` — `python3 server.py` → :3456. Affiche `provider_pubkey` et signe les réponses (`X-Provider-Signature`).
...
## 2b · Stake et sécurité (prod)

En prod : **stake** (≥ $10), limites de publication, confiance **LUMEN**, réponses Ed25519. Déposez le stake d'abord :
...
CLI, token, stake (prod), puis publish.
...
ARGUS filtre sous `ARGUS_MIN_HUB_TRUST` (0.25).
...
- [User guide (Français)](../user-guide/fr.md)
