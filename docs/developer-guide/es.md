# Publica una capability en 15 minutos (Español)

> servidor HTTP · manifiesto · CLI Hub · USDC de ARGUS
...
`hello-capability` — `python3 server.py` → :3456. Imprime `provider_pubkey` y firma respuestas (`X-Provider-Signature`).
...
## 2b · Stake y seguridad (prod)

En producción: **stake** (≥ $10), límites de publicación, confianza **LUMEN**, respuestas firmadas Ed25519. Deposita stake primero:
...
`pip install -e aimarket-hub/`, token, stake (prod), luego publish.
...
ARGUS filtra listings bajo `ARGUS_MIN_HUB_TRUST` (0.25).
...
- [User guide (Español)](../user-guide/es.md)
