# Objavite capability za 15 minuta (Hrvatski)

> HTTP poslužitelj · manifest · Hub CLI · USDC od ARGUS-a
...
`hello-capability` — `python3 server.py` → :3456. Ispisuje `provider_pubkey`; potpisuje odgovore (`X-Provider-Signature`).
...
## 2b · Stake i sigurnost (prod)

Prod: **stake** (≥ $10), limiti objave, **LUMEN** trust, Ed25519 potpisani odgovori. Prvo stake:
...
CLI, token, stake (prod), zatim publish.
...
ARGUS filtrira ispod `ARGUS_MIN_HUB_TRUST` (0.25).
...
- [User guide (Hrvatski)](../user-guide/hr.md)
