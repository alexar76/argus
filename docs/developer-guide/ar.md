# انشر capability في 15 دقيقة (العربية)

> خادم HTTP · manifest · Hub CLI · USDC من ARGUS
...
`hello-capability` — `python3 server.py` → :3456. يطبع `provider_pubkey` ويوقّع الردود (`X-Provider-Signature`).
...
## 2b · الضمان والأمان (prod)

في الإنتاج: **stake** (≥ $10)، حدود النشر، ثقة **LUMEN**، توقيع Ed25519. أودع stake أولاً:
...
ثبّت CLI، عيّن token، stake في prod، ثم publish.
...
ARGUS يصفّي القوائم دون `ARGUS_MIN_HUB_TRUST` (0.25).
...
- [User guide (العربية)](../user-guide/ar.md)
