# Xuất bản capability trong 15 phút (Tiếng Việt)

> máy chủ HTTP · manifest · Hub CLI · USDC từ ARGUS
...
`hello-capability` — `python3 server.py` → :3456. In `provider_pubkey`; ký phản hồi (`X-Provider-Signature`).
...
## 2b · Stake & bảo mật (prod)

Prod: **stake** (≥ $10), giới hạn publish, tin cậy **LUMEN**, phản hồi Ed25519. Nạp stake trước:
...
CLI, token, stake (prod), rồi publish.
...
ARGUS lọc dưới `ARGUS_MIN_HUB_TRUST` (0.25).
...
- [User guide (Tiếng Việt)](../user-guide/vi.md)
