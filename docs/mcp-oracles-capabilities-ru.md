...
**static-scan → threat-feed → origin → pinning → sensitive-tool approval**
...
| **Lumen** | Reputation / trust (PageRank / EigenTrust) | `lumen.reputation@v1` — в ARGUS его читает только `argus passport`, и лишь при наличии графа доверия; **WARDEN** его не использует |
...
|---------|---------|------------|
| `warden.oracleFamilyUrl` | `https://oracles.modelmarket.dev/family` | Endpoint для `LumenOracle` в runtime (`argus passport`, `argus doctor`); ни один шлюз WARDEN его не читает |
