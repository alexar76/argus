...
Он построен на двух слоях стека, которых обычно нет у типовых MCP-клиентов:

1. **🛡️ WARDEN** — MCP-файрвол безопасности, который проверяет сторонние
   серверы по тому, что может проверить сам: многоуровневое (tiered) статическое
   сканирование определений инструментов, подписанная лента угроз, происхождение
   объявления сервера и дрейф относительно закреплённого снимка. **Работает без кошелька, без блокчейна
   и без сети.**
...
|---|---|---|
| 🛡️ **Файрвол WARDEN** | Каждый MCP-сервер проверяется цепочкой шлюзов — статическое сканирование определений инструментов → лента угроз → **происхождение (`origin`)** → закрепление определений (def-pinning) — прежде чем сработает хотя бы один инструмент. | Отравление инструментов, rug-pull (дрейф определений), эксфильтрация и сбор учётных данных блокируются *по умолчанию*. Каждый шлюз решает по данным, которые ARGUS уже держит локально, поэтому проверка не требует сети, и вердикт нельзя «переубедить». |
...
> WARDEN поставляется отдельно как **[`@aimarket/warden`](https://github.com/alexar76/warden)** —
> библиотека без зависимостей, которую можно поставить перед своим MCP-хостом, не переходя на ARGUS.
> ARGUS зависит от этого пакета; этот раздел — о том, что он делает внутри агента.
...
- **Статическое сканирование** — сигнатуры инъекций / эксфильтрации / сбора
  секретов / скрытого unicode в определениях инструментов. Правила **разделены на
  tier**: *блокирующие* покрывают формулировки без безобидного прочтения;
  *advisory* (параметр `api_key`, упоминание `.env`, «you must») сообщаются, но
  никогда не блокируют и не влияют на score — именно это и пропускает честные
  серверы. Каждый вердикт называет версию и digest ruleset, который его выдал.
- **Лента угроз** — встроенный deny-list + опциональная подписанная удалённая лента.
- **Происхождение (`origin`)** — откуда взялось объявление сервера: вы объявили
  его сами в `mcp.servers` или он обнаружен в удалённом каталоге из
  `mcp.catalogs`. При `allowUnknownServers: false` сервер, найденный в каталоге,
  блокируется (`SERVER_UNDECLARED`); ваши собственные объявления проходят
  **всегда**, поэтому fail-closed по-прежнему оставляет способ подключиться.
...
- **Квесты и бейджи** — *First Blood* (первый выигрыш в лотерею), *Rainmaker*
  ...
  *Trusted* и ранг по репутации **сегодня заработать нельзя**: для LUMEN-score
  нужен граф доверия, а ни одна инсталляция не публикует рёбра доверия, по
  которым ARGUS мог бы считаться.
- **Flex Card** — `argus flex` (или `/flex` в Telegram) рисует стильную карточку
  ...
  ранг по репутации (пустой, пока нет графа доверия). Цифры + эмодзи = нет
  языкового барьера → делитесь где угодно.
- **Глобальный лидерборд** *(по согласию, opt-in)* — соревнуйтесь с агентами по
  всему миру по XP, заработку или бережливости.

Каждая метрика **настоящая** — это ваша реальная экономика и показатели
бережливости, вычисленные локально из собственной памяти вашего
...
> *продаёт* на нём и *защищает* пользователя от цепочки поставок MCP — с помощью
> WARDEN, который решает локально и не обращается ни к одному оракулу.
...
шлюзов WARDEN (статика + угрозы + origin + закрепление), память/уроки,
