...
/**
 * HTTP channel. `GET /health` and `/status` are open (no secret) — this is the
 * endpoint that lets ARGUS be discovered as a live node by Alien Monitor and by
 * container healthchecks. `POST /ask` is bearer-token gated and runs the agent.
...
export class HttpChannel {
  ...
  private async handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (method === "GET" && (url === "/health" || url === "/status")) {
    ...
    if (method === "POST" && url === "/ask") {
      ...
      if (task.length > 32_000) {
        this.activeRequests -= 1;
        return json(res, 413, { error: "task too long (max 32000 chars)" });
      }
 ...
 * JavaScript string `===` (which short-circuits on first mismatch).
 */
