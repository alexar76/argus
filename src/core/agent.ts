...
import { isSensitiveTool, tryCanonicalToolsHash, UNCANONICAL_TOOLS_HASH } from "@aimarket/warden";
...
import { inspectTask, wrapUntrustedTask } from "../aegis/promptFirewall.js";
...
export class Agent {
  ...
  async run(
    ...
    const firewall = inspectTask(task);
    if (firewall.reject) {
      log.warn(`prompt firewall blocked task: ${firewall.reason ?? "injection"}`);
      const budget = new Budget(config.budget);
      const startedAt = new Date().toISOString();
      const toolsHash = tryCanonicalToolsHash(this.deps.tools.map((t) => t.def)) ?? UNCANONICAL_TOOLS_HASH;
    ...
    const audit: RunAudit = {
      // A bridged MCP schema carrying a non-integer number has no canonical form
      // (SPEC §4.3), so it cannot be committed to reproducibly. Record the marker
      // rather than crash the run or seal bytes no verifier could recompute —
      // WARDEN's pinning gate has already reported it as TOOL_DEF_UNCANONICAL.
      toolsHash: tryCanonicalToolsHash(toolDefs) ?? UNCANONICAL_TOOLS_HASH,
      chain: [],
      spend: [],
      verifications: [],
      baselines: new Map(),
    ...
    const safeTask = wrapUntrustedTask(task);
    let messages: Message[] = [...prior, { role: "user", content: safeTask }];
    let answer = "";
