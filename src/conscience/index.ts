...
import { toVerifyCertArtifact, type VerifyCertSeal } from "../verifycert/index.js";
...
export interface ConscienceInput {
  ...
  /** Pay-on-Verified verdicts for paid hires — hub envelopes + rejection receipts, sealed verbatim. */
  verifyCert?: VerifyCertSeal;
...
export function buildConscienceBundle(input: ConscienceInput): ConscienceBundle {
  ...
  if (input.verifyCert) {
    artifacts.push(toVerifyCertArtifact(input.verifyCert));
  }
  if (input.selfBond) {
