...
export function printFindings(
  findings: { severity: string; code: string; message: string; tool?: string; advisory?: boolean }[],
): void {
  for (const f of findings) {
    if (f.severity === "info") continue;
    // Advisory findings never blocked anything; saying so stops a reader from
    // reading "[medium]" next to an allowed connection as a contradiction.
    const label = f.advisory ? `${f.severity} · advisory` : f.severity;
    console.log(`    [${label}] ${f.code}${f.tool ? ` (${f.tool})` : ""}: ${f.message}`);
  }
}
