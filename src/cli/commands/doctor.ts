...
import { egressPosture, productionMode } from "../../config.js";
...
export async function cmdDoctor(config: ArgusConfig, path?: string): Promise<number> {
  ...
  const lines = [
    ...
    `  warden:     blockAt=${config.warden.blockAtSeverity} pin=${config.warden.pinToolDefs} allowUndeclared=${config.warden.allowUnknownServers} oracle=${config.warden.oracleFamilyUrl}`,
    `  hardened:   ${productionMode() ? "AIFACTORY_PROD=1 — laptop defaults tightened wherever argus.config.json is silent" : "no (AIFACTORY_PROD unset)"}`,
    `  egress:     ${egressPosture(config).label}`,
