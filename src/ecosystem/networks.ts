...
/**
 * Real Base-mainnet deployment, GENERATED from the canonical registry
 * (`config/deployments/base-mainnet.json`) by
 * `scripts/sync_deployment_addresses.py` — which also owns the registry-name →
 * field-name mapping. These addresses used to be typed in here by hand, so a
 * redeploy meant editing the same values in three languages and hoping the
 * drift test caught whichever copy was forgotten. Do not inline an address:
 * edit the registry and re-run the sync.
 */
export const BASE_MAINNET_ADDRESSES: Addresses = baseDeployment.addresses as Addresses;
