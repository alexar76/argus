 ...
 * fail, so oracle downtime can never break local autonomy.
 *
 * Every degraded result carries a `reason` saying whether the oracle was actually
 * consulted. Callers MUST NOT describe `no-graph-data` as an outage: nothing was
 * requested. Today NO caller supplies edges (see `scoreEntity`), so in practice
 * every lookup is `no-graph-data` and the scored path below is exercised only by
 * callers that build a graph themselves.
 */
export class LumenOracle implements TrustOracle {
  ...
  async scoreEntity(entityId: string, edges?: TrustEdge[]): Promise<ReputationScore> {
    ...
    // and, crucially, say that we never asked. No request is made on this path.
    if (!edges || edges.length === 0) {
      return {
        score: 0.5,
        degraded: true,
        reason: "no-graph-data",
        detail: `no trust edges supplied for "${entityId}"; the oracle was not contacted`,
      };
    }
    ...
    try {
      ...
      if (!scores || scores.length === 0 || target >= scores.length) {
        return {
          ...
          degraded: true,
          reason: "oracle-error",
          detail: "oracle responded without a usable scores array for the target node",
        };
      }
      ...
      // A request WAS attempted and failed — this is the only path allowed to
      // describe the oracle as unreachable.
      const detail = (err as Error).message;
      this.log.debug(`LUMEN unreachable, neutral score: ${detail}`);
      return { score: 0.5, degraded: true, reason: "oracle-error", detail };
    }
  }
