...
export function resolveArgusHeartbeatEndpoint(baseUrl: string): string {
  const trimmed = baseUrl.replace(/\/+$/, "");
  return `${trimmed}/api/argus/heartbeat`;
}
...
/**
 * Presence ping for the ARGUS · N roster. Fail-soft like run push.
 */
export async function pushHeartbeatToMonitor(
  payload: MonitorHeartbeatPayload,
  cfg: MonitorFeedConfig,
  log: Logger,
): Promise<void> {
  const base = validateMonitorBaseUrl(cfg.url);
  const token = cfg.token.trim();
  if (!base || !token) return;
  if (!payload.instance_id?.trim()) return;

  const endpoint = resolveArgusHeartbeatEndpoint(base);
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), PUSH_TIMEOUT_MS);
  try {
    const res = await fetch(endpoint, {
    ...
    if (!res.ok) log.debug(`monitor heartbeat: POST ${endpoint} → ${res.status}`);
  } catch (err) {
    const msg = (err as Error).message;
    if (msg.includes("abort")) log.debug("monitor heartbeat: timed out");
    else log.debug(`monitor heartbeat: ${msg}`);
  } finally {
    clearTimeout(timer);
  }
}
