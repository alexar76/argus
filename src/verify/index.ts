...
import { tryCanonicalToolsHash } from "@aimarket/warden";
...
export interface ClaimResult {
  ...
  detail: string;
  /**
   * Was the verifying key established INDEPENDENTLY of the artifact?
   *
   * A signature check answers "are these bytes internally consistent under the key that
   * came with them" — which a fabricated bundle satisfies trivially, because the forger
   * supplies both. `ok` alone therefore never meant "this oracle signed this"; only a key
   * from outside the artifact can mean that. `undefined` for claims where the question does
   * not arise (a self-recomputed SHA-256 has nothing to attribute).
   */
  attributed?: boolean;
}
...
export interface VerifyOptions {
  /**
   * Signer keys this caller trusts, established independently of the artifact — e.g.
   * fetched from the capability's `source_hub` `/.well-known/ai-market.json`, which
   * `OracleClient.fetchSignerKey` already does, or pinned in config.
   *
   * When supplied, a signature claim whose key is not among these FAILS: an artifact
   * carrying its own key is exactly the thing being guarded against. When omitted, the
   * signature is still checked but the claim is reported as unattributed, so neither the
   * text nor a programmatic consumer can read it as provenance.
   */
  trustedSignerKeys?: readonly string[];
}

/** Is `key` one the caller vouched for from outside the artifact? */
function keyIsTrusted(key: string, opts?: VerifyOptions): boolean | undefined {
  const trusted = opts?.trustedSignerKeys;
  if (!trusted || trusted.length === 0) return undefined;
...
export function verifyArtifact(a: VerifiableArtifact, opts?: VerifyOptions): ClaimResult[] {
  switch (a.type) {
    case "oracle-receipt": {
      const r = a.receipt ?? {};
      ...
      const trust = keyIsTrusted(a.signerPublicKey, opts);
      if (trust === false) {
        return [{
          type: a.type, label, ok: false, attributed: false,
          detail: "receipt is signed by a key this verifier does not trust — an artifact that carries its own key attributes nothing",
        }];
      }
      const ok = verifyEd25519(receiptCanonical(r), value, a.signerPublicKey);
      if (!ok) {
        return [{ type: a.type, label, ok: false, attributed: false, detail: "Ed25519 signature INVALID — receipt was forged or altered" }];
      }
      return [{
        type: a.type, label, ok: true, attributed: trust === true,
        detail: trust === true
          ? `Ed25519 signature valid, signed by a trusted key (${cid})`
          : `Ed25519 signature self-consistent (${cid}) — UNATTRIBUTED: the key came with the artifact, so this proves the bundle is internally consistent, not who produced it. Pass trustedSignerKeys to attribute it.`,
      }];
    }
    case "commitment": {
    ...
    case "tool-pin": {
      ...
      // A tool set with no canonical form cannot be re-hashed, so the claim cannot
      // be checked — which for a verifier is a failure, never a pass.
      const got = tryCanonicalToolsHash(tools as ToolDef[]);
      if (got === undefined) {
        return [{ type: a.type, label, ok: false, detail: "tool list has no canonical form (non-integer number in a schema) — cannot re-hash, claim unverifiable" }];
      }
      const ok = got === a.hash;
    ...
    case "sealed-chain": {
      ...
      if (a.seal) {
        const sealKey = typeof (a.seal as { publicKey?: unknown }).publicKey === "string"
          ? String((a.seal as { publicKey?: unknown }).publicKey)
          : "";
        const sealTrust = sealKey ? keyIsTrusted(sealKey, opts) : undefined;
        if (sealTrust === false) {
          return [{
            type: a.type, label, ok: false, attributed: false,
            detail: `consent chain intact (${links}) but the head seal is signed by a key this verifier does not trust`,
          }];
        }
        const sealOk = verifySealedChain(chain, a.seal);
        if (!sealOk) {
          return [{ type: a.type, label, ok: false, attributed: false, detail: "chain re-derives but the Ed25519 head seal is INVALID — head re-signed or swapped" }];
        }
        return [{
          type: a.type, label, ok: true, attributed: sealTrust === true,
          detail: sealTrust === true
            ? `consent chain intact (${links}) + Ed25519 head seal valid under a trusted key`
            : `consent chain intact (${links}) + head seal self-consistent — UNATTRIBUTED: the seal's key came with the artifact. Pass trustedSignerKeys to attribute it.`,
        }];
      }
    ...
    case "attestation": {
      ...
      const detail = ok
        ? `Ed25519 attestation valid — claims held: ${att.claims.join(", ") || "(none)"} (CAVEAT: signature checks against the attestation's own embedded key, which is self-asserted — NOT bound to a pre-trusted ARGUS/oracle identity; anchor it to a key you already trust before relying on it)`
        : !sigOk
...
export function verifyBundle(input: unknown, opts?: VerifyOptions): VerifyReport {
  let arts: unknown[];
  ...
  const claims = arts.flatMap((a) => verifyArtifact(a as VerifiableArtifact, opts));
