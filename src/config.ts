...
/**
 * Parse a positive-integer env var, or `undefined`. A typo (`"soon"`, `"0"`, `"-1"`)
 * yields `undefined` so the consumer's own default applies — never a silently
 * disabled limit.
 */
function positiveIntEnv(v: string | undefined): number | undefined {
...
export interface WardenConfig extends WardenPolicy {
  /**
   * Signed threat-feed endpoint. **No default** — ARGUS ships no feed URL, so
   * unless an operator sets this (and `feedPublicKey`) the threat gate is exactly
   * the built-in deny-list inside `@aimarket/warden`.
   */
  threatFeedUrl?: string;
  /** Ed25519 public key (hex-encoded SPKI DER) for verifying signed threat feeds. */
  feedPublicKey?: string;
  /**
   * Maximum age of a signed feed's timestamp before it is refused as stale (a
   * replay guard). Unset → the 24 h default inside `@aimarket/warden`.
   */
  feedMaxAgeMs?: number;
  ...
  return {
    ...
    warden: {
      ...
      sensitiveToolPatterns: [
        ...
        "*invoke*", "*trade*", "*swap*", "*buy*", "*enter*", "*spend*", "*approve*", "*withdraw*",
      ],
      allowUnknownServers: true,
      pinToolDefs: true,
      oracleFamilyUrl: process.env.ARGUS_ORACLE_FAMILY_URL ?? "https://oracles.modelmarket.dev/family",
      // Deliberately no defaults: a feed endpoint baked into the shipped config
      // would be a single point every install has to trust, and a feed without a
      // pinned publisher key is refused anyway. With both unset the threat gate is
      // the built-in deny-list only — documented in docs/security-warden.md.
      threatFeedUrl: process.env.ARGUS_THREAT_FEED_URL || undefined,
      feedPublicKey: process.env.ARGUS_THREAT_FEED_PUBKEY || undefined,
      feedMaxAgeMs: positiveIntEnv(process.env.ARGUS_THREAT_FEED_MAX_AGE_MS),
...
export interface LoadedConfig {
  config: ArgusConfig;
  path?: string;
  /** Defaults tightened by production mode, in human-readable form. See hardenForProduction. */
  hardened: string[];
}

/**
 * The ecosystem-wide production marker, the same one the hub, GAIA, ATLAS and the
 * factory read. ARGUS's own defaults are tuned for a laptop; this is how a host says
 * it is not one.
 */
export function productionMode(): boolean {
  return ["1", "true", "yes", "on"].includes((process.env.AIFACTORY_PROD ?? "").trim().toLowerCase());
}

/**
 * Harden the defaults that are right on a laptop and wrong on an unattended host.
 *
 * `allowUnknownServers: true` exists so catalog discovery works the moment you install
 * ARGUS — a first run that blocks every server it finds is a first run nobody repeats.
 * On a host running unattended, possibly with a funded wallet, it means the opposite:
 * any server the catalog happens to list gets its tools bridged into an agent that can
 * spend.
 *
 * Applied ONLY where argus.config.json is silent. An operator who wrote
 * `"allowUnknownServers": true` meant it, and silently reversing that would be worse
 * than the permissive default: they would believe discovery works, and it would not,
 * and nothing would say why. Returns what it changed so `doctor` can print it — a
 * hardening nobody can see is one nobody can debug.
 */
/**
 * Whether `web_fetch` is gated, and by what. ONE definition, because runtime builds the
 * guard from it and `doctor` prints it, and a posture that reports differently from how
 * it behaves is worse than either answer alone.
 *
 * No allowlist used to mean NO GUARD — the absence of a decision became the most
 * permissive setting. In production the absence of a decision gates instead, with an
 * empty allowlist, and EgressGuard refuses everything with a legible reason. `*` is how
 * an operator says "unrestricted" out loud; it must be spelled, because EgressGuard
 * treats a bare `*` as a hostname matching nothing.
 */
export function egressPosture(cfg: ArgusConfig): { gated: boolean; allowlist: string[]; label: string } {
  const raw = cfg.warden.egressAllowlist ?? [];
  const unrestricted = raw.some((h) => h.trim() === "*");
  const allowlist = raw.filter((h) => h.trim() !== "*");
  if (unrestricted) {
    return { gated: false, allowlist, label: "UNRESTRICTED (ARGUS_EGRESS_ALLOWLIST=*, stated explicitly)" };
  }
  if (allowlist.length > 0) {
    return { gated: true, allowlist, label: `allowlist: ${allowlist.join(", ")}` };
  }
  if (productionMode()) {
    return {
      gated: true,
      allowlist,
      label: "BLOCKED — no allowlist under AIFACTORY_PROD=1; set ARGUS_EGRESS_ALLOWLIST (or =* to allow all)",
    };
  }
  return { gated: false, allowlist, label: "unrestricted (no allowlist, not production)" };
}

export function hardenForProduction(cfg: ArgusConfig, stated: Partial<WardenConfig>): string[] {
  if (!productionMode()) return [];
  const applied: string[] = [];
  if (!("allowUnknownServers" in stated) && cfg.warden.allowUnknownServers) {
    cfg.warden.allowUnknownServers = false;
    applied.push(
      "warden.allowUnknownServers=false — a catalog-discovered server must be declared in mcp.servers",
    );
  }
  return applied;
}
...
export function loadConfig(explicitPath?: string): LoadedConfig {
  ...
  let merged = base;
  // What the FILE said, as opposed to what the merge produced. Production hardening
  // needs the difference: a key the operator wrote is a decision, a key they left out
  // is a default, and only the second may be tightened underneath them.
  let statedWarden: Partial<WardenConfig> = {};

  if (path) {
    try {
      const raw = JSON.parse(readFileSync(path, "utf8")) as RawConfig;
      statedWarden = raw.warden ?? {};
  ...
  const hardened = hardenForProduction(merged, statedWarden);
  ...
  return { config: merged, path, hardened };
}
