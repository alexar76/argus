...
Está construido sobre dos capas del stack que los clientes MCP típicos no suelen tener:

1. **🛡️ WARDEN** — un firewall de seguridad MCP que examina servidores de terceros
   con lo que puede comprobar por sí mismo: un escaneo estático escalonado de las
   definiciones de herramientas, un feed de amenazas firmado, de dónde vino la
   declaración del servidor y la deriva contra un snapshot fijado. **Funciona sin wallet, sin cadena y sin red.**
...
|---|---|---|
| 🛡️ **Firewall WARDEN** | Cada servidor MCP es examinado por una cadena de compuertas — escaneo estático de tool-def → feed de amenazas → **origin** → fijado de definiciones — antes de que se ejecute una sola herramienta. | El envenenamiento de herramientas, los rug-pulls (deriva de definiciones), la exfiltración y la recolección de credenciales se bloquean *por defecto*. Cada compuerta decide sobre evidencia que ARGUS tiene localmente, así que el vetting no necesita red y nadie puede hablarle para cambiar su veredicto. |
...
flowchart TB
  subgraph OFF["Corre offline — sin AICOM, sin wallet"]
    ...
    L4["🛡️ Capa 4 · Host MCP + WARDEN — estático · amenazas · origin · fijado"]
    L1 --- L2 --- L3 --- L4
  end
...
> WARDEN se distribuye por separado como **[`@aimarket/warden`](https://github.com/alexar76/warden)**:
> una biblioteca sin dependencias que puedes poner delante de tu propio host MCP sin adoptar ARGUS.
> ARGUS depende de ese paquete; esta sección es lo que hace dentro del agente.
...
```mermaid
flowchart LR
  S[servidor MCP] --> A[1 · escaneo estático] --> B[2 · feed de amenazas] --> C[3 · origin] --> D[4 · fijado de def] --> V{¿permitir?}
  V -->|sí| OK[herramientas puente<br/>fijar defs]
  V -->|no| NO[bloquear + reportar]
```

- **Escaneo estático** — firmas de inyección / exfiltración / recolección de secretos / unicode oculto en las tool-defs. Las reglas están **escalonadas** (*tiered*): las reglas *bloqueantes* cubren formulaciones sin lectura benigna; las reglas *advisory* (un parámetro `api_key`, una mención de `.env`, un «you must») se reportan pero nunca bloquean ni afectan al score — que es lo que deja pasar a los servidores honestos. Cada veredicto nombra la versión + digest de la ruleset que lo produjo.
- **Feed de amenazas** — deny-list integrada + feed remoto firmado opcional.
- **Origin** — de dónde vino la declaración: declarado por ti bajo `mcp.servers`, o descubierto desde un catálogo remoto bajo `mcp.catalogs`. Con `allowUnknownServers: false` un servidor descubierto en un catálogo se bloquea (`SERVER_UNDECLARED`); tus propias declaraciones siempre pasan, así que fail-closed sigue conectando.
...
- **Misiones e insignias** — *First Blood* (primera victoria en la lotería),
  ...
  *Trusted* y el rango de reputación **no son obtenibles hoy**: una puntuación LUMEN
  necesita un grafo de confianza, y ningún despliegue publica aristas de confianza
  contra las que ARGUS pueda puntuar.
- **Flex Card** — `argus flex` (o `/flex` en Telegram) renderiza una carta elegante y
  ...
  reputación (en blanco hasta que exista un grafo de confianza). Números + emoji = sin
  barrera de idioma → compártela donde sea.
- **Tabla de clasificación global** *(opt-in)* — compite contra agentes de todo el
  mundo por XP, ganancias o frugalidad.

Cada estadística es **real** — es tu economía y tu rendimiento de frugalidad
...
> en él y *defiende* al usuario contra la cadena de suministro MCP — con WARDEN,
> que decide localmente y no consulta ningún oráculo.
...
npm test              # vitest (gobernador de presupuesto, host MCP + conexión de WARDEN, mapeo de proveedores)
...
WARDEN (estático + amenazas + origin + fijado), memoria/lecciones, host MCP,
