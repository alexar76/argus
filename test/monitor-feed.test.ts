...
describe("monitor feed", () => {
  ...
  it("validateMonitorBaseUrl rejects non-http schemes and embedded credentials", () => {
    expect(validateMonitorBaseUrl("https://monitor.modelmarket.dev")).toBe(
  ...
  it("resolveArgusRunEndpoint appends /api/argus/run", () => {
    ...
    expect(resolveArgusRunEndpoint("https://monitor.modelmarket.dev")).toBe(
      "https://monitor.modelmarket.dev/api/argus/run",
    );
  });
