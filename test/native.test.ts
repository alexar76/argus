...
import { isSensitiveTool } from "@aimarket/warden";
...
const policy: WardenPolicy = {
  ...
  pinToolDefs: true,
  sensitiveToolPatterns: ["*invoke*", "*trade*", "*swap*", "*buy*", "*enter*", "*spend*", "*approve*", "*withdraw*", "*delete*", "*write*", "*exec*", "*shell*", "*payment*", "*transfer*", "*email*", "*send*"],
};
...
describe("native ecosystem tools", () => {
  it("exposes the full first-party toolset on a chain + crypto (live)", () => {
    ...
    for (const n of ["oracle_call", "oracle_random", "lottery_status", "lottery_enter", "lottery_buy", "acex_status", "acex_trade", "hub_discover", "hub_invoke"]) {
  ...
  it("crypto OFF + no chain (test mode) exposes only free oracle tools", () => {
    ...
    for (const n of ["lottery_buy", "lottery_enter", "lottery_status", "acex_trade", "acex_status", "hub_invoke", "hub_discover"]) {
  ...
  it("UNI (chain present, public crypto OFF) exposes lottery + ACEX, but NOT the paid hub tools", () => {
    ...
    for (const n of ["lottery_status", "lottery_enter", "lottery_buy", "acex_status", "acex_trade"]) {
      expect(names).toContain(n);
    }
  ...
  it("spend tools self-guard: acex_trade/lottery_buy/lottery_enter need a wallet; hub tools need crypto", async () => {
    ...
    expect((await tools.find((t) => t.def.name === "lottery_enter")!.run({}, { ...ctx, approved: true })).ok).toBe(false);
...
describe("spending tools are sensitive; read tools are not", () => {
  it("flags only value-moving tools for approval", () => {
    for (const n of ["lottery_buy", "lottery_enter", "hub_invoke", "acex_trade"]) expect(isSensitiveTool(n, policy)).toBe(true);
